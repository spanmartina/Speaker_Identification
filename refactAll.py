...
def enroll(name, file):
    ...
    # Path
    path = os.path.join(p.source_train, name) #create folder and save audio there (directory = name)
    #create directory
    try: 
        os.mkdir(path) 
...
def featureExtraction_allUsers():
    features = np.asarray(()) #we created Array
    # path = os.path.join(source, directoryName) 
    ...
    for x in sourceFolder:
        for name in os.listdir(p.source_train + x): #TrainingData/x where x is the folder in it. This function will work for each folder.
...
# print("Username")
# username = input() 
# print("File uploaded(without wav)")
# filename = input()
# enroll(username,filename)

# featureExtraction('Ethan')